...
CONSTEXPR_INLINE bool is_SPACE(char c) { return c == ' ' || c == '\t'; }
CONSTEXPR_INLINE bool is_SEP(char c)   { return c == ',' || c == ';'; }
CONSTEXPR_INLINE bool is_LF(char c)    { return c == '\n'; }
CONSTEXPR_INLINE bool is_EOS(char c)   { return c == EOSTR; }

CONSTEXPR_INLINE bool is_SPACE_LF(char c)         { return is_SPACE(c) || is_LF(c); }
CONSTEXPR_INLINE bool is_SPACE_LF_EOS(char c)     { return is_SPACE_LF(c) || is_EOS(c); }
CONSTEXPR_INLINE bool is_SPACE_SEP_LF_EOS(char c) { return is_SPACE_LF_EOS(c) || is_SEP(c); }
CONSTEXPR_INLINE bool is_LF_EOS(char c)           { return is_LF(c) || is_EOS(c); }
CONSTEXPR_INLINE bool is_SEP_LF_EOS(char c)       { return is_SEP(c) || is_LF_EOS(c); }
