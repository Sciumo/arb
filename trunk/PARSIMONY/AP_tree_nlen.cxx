...
void AP_tree_nlen::exchange(AP_tree_nlen *other) {
    // exchange 'this' with other
    // 'this' has to be in tree; other has to be a "single node"
    //
    // Used by quick-add.

    AP_tree_root *root = get_tree_root();
    ap_assert(root);
    ap_assert(!other->get_tree_root());

    ASSERT_VALID_TREE(rootNode());
    ASSERT_VALID_TREE(other);

    ap_main->push_node(this, STRUCTURE);
    ap_main->push_node(other, STRUCTURE);
    ap_main->push_node(get_father(), BOTH);
    push_all_upnode_sequences(get_father());

    AP_tree_edge *myEdge    = nextEdge(NULL);
    AP_tree_nlen *connected = myEdge->otherNode(this);
    myEdge->unlink();

    if (is_leftson()) {
        father->leftson = other;
    }
    else {
        father->rightson = other;
    }
    other->father = father;
    father        = NULL;

    other->set_tree_root(root);
    set_tree_root(NULL);

    myEdge->relink(other, connected);
